...
> 创建日期：2026-08-07
> 数据来源：`X:\web\projects\musicfree-plugins\etc\` 目录下的 23 个插件文件
...
本文对 etc/ 下全部 23 个插件逐一编号（P-01 ~ P-23），按 QQ 系 / 酷我系 / 聚合与歌词 三大类组织。每个插件给出：
...
**API 端点**
- 搜索/专辑/歌手：与 P-01 同族（u.y.qq.com）
- **getMediaSource**：`https://musicapi.haitangw.net/...`（元力菌生态，无鉴权）

**getMediaSource 范式**：范式 A，无鉴权。音质 `low/standard/high→exhigh, super→lossless`。

**架构特点**：代码部分混淆，端点可提取但解析逻辑细节需实测。无测速、无 VIP 检测。

**可借鉴点**：`musicapi.haitangw.net` 端点（与 P-04 回退链的 `music.haitangw.cc` 不同域名，疑似同生态不同实例）。
...
**API 端点**：代码混淆，推测直连酷我官方，无显式第三方端点可提取。

**音质映射**：`low→128k, standard→320k, high→flac, super→hires`

**架构特点**：全混淆，无法提取有效端点/密钥。无测速、无 VIP 检测（即使有也读不出）。

**可借鉴点**：无（混淆不可读）。

---

### P-15b · `元力QQ v1.2.0.js` / `元力KW v1.2.0.js`（长青SVIP音源，付费中转）⭐

| 项 | 值 |
|----|----|
| 文件名 | `元力QQ v1.2.0.js` / `元力KW v1.2.0.js` |
| @name | **长青SVIP音源**（`@version 1.0.0`，platform version 1.2.0） |
| author | `微信公众号:元力菌` |
| srcUrl | QQ `https://13413.kstore.vip/yuanli/qq.js`；KW `https://13413.kstore.vip/yuanli/kw.js` |
| 打包 | 混淆（`_0x4a03` 自定义 base64 + `_0x2637` base64+RC4），module.exports |
| 依赖 | axios + he（QQ 另含 crypto-js） |
...
**getMediaSource 端点（§4.1 表 #19/#20，均无鉴权、实测可用）**
- **元力QQ**：`http://175.27.166.236/kgqq1/qq.php?id={songmid}&type=json&level={...}` → `{code:200,msg:'换源成功',data:{url,...}}`；`level` 映射 `low/standard/high→exhigh`、`super→lossless`；**返回的是 kuwo CDN**（`car-lv/car-er.kuwo.cn`，M800=320k mp3、F000=flac），即元力后端对 QQ 走"跨平台酷我重搜"兜底。
- **元力KW**：`https://music.haitangw.cc/music1/kw.php?id={rid}&level={...}` → `{code:200,msg:'解析成功',data:{url}}`；`level` 映射 `low/standard→exhigh(320k)`、`high/super→lossless(flac)`。

**元数据接口（search/lyric/专辑/榜单/歌单）**：走各平台**官方公开匿名端点**（QQ：`u.y.qq.com/musicu.fcg`、`c.y.qq.com` 等；KW：`search.kuwo.cn/r.s`、`m.kuwo.cn` 等），均带 `Cookie:'uin='` 占位 + `g_tk=5381`，无真实登录态也可用。

**音质标准**：元力 系标准仅 `exhigh(320k) / lossless(FLAC)`，**无母带/Hi-Res**（hires/master/atmos 等参数经 `music.haitangw.cc` 中转实测全部塌缩回 320k，2026-08-07 三首样本验证）。母带级需走 musicdl 内置 `kwdec.liuyunidc.cn`，其标准 `master/atmos_plus/atmos/flac/320k` 可出至臻母带（见 §4.8；`kuwo.py` 已由 `MUSIC_QUALITIES[3:]` 改为 `[:]` 解锁母带）。

**已知插件缺陷**：QQ `getAlbumInfo` 的 `albumID` 恒传 0，实际返回空列表（bug）；QQ `getLyric` 与 KW 歌词接口在部分歌曲返回空/报异常（歌词接口对传入对象字段依赖强）。

**可借鉴点**：① 元力私有中转是"瘦客户端 + 服务端凭证"架构的又一实例（青听同款，读 4.8.6）；这两个中转端点**无鉴权可用**，可作 musicdl 低优先级兜底源（⚠️ 付费私有、非公开、随时失效，勿作主源）；② QQ→kuwo 跨平台兜底在此插件再次出现，佐证 P-04/青听的同款策略在业界常用，musicdl `qq.py` 值得照搬。
...
etc/ 下另有若干参考文件（如旧版酷狗、网易云备份等），非本次 QQ/酷我专题范围，从略。
...
| 3 | `api.nki.pw/API/music_open_api.php?apikey=KEY&mid={mid}` | QQ | P-01/P-06 | apikey | 一次返回全音质 | ✅ 已有（`_parsewithnkiapi`） | ✅ 可用（响应 8s，偏慢） |
| 4 | `cyapi.top/API/qq_music.php?apikey=KEY&mid={mid}` | QQ | P-03 | apikey 硬编码 | 忽略音质 | ✅ 已有（`_parsewithcyapi`） | ✅ 可用 |
...
| 6 | `musicapi.haitangw.net/music/{kw|wy|kg}.php` | 酷我/网易/酷狗 | P-05 | 无 | `exhigh/lossless` | ✅ 已有（`_parsewithhaitangwapi`，kuwo/kugou/netease 均内置此域名） | ❌ **404**（路径推测错误或已下线） |
...
> **实测结论（2026-08-07，样本 QQ `0019tNGN1TJbLT` / 酷我 `392634142`）**：14 个端点中 **7 个可用**（#2 lx-kw 全音质、#3 nki、#4 cyapi、#8 lerd 303、#10 nmobi、#12 nxinxz、#13 haitangw.cc），**4 个不可用**（#5 ikunshare DNS 挂、#6 haitangw.net 404、#7 lxmusicapi 403、#9 kwdec DNS 挂），#1 lx-tx **128k 可用但 320k/flac 全部 code 2**（疑似服务端高音质受限），#14 yunzhiapi **需注册 token**（文档此前记"无"鉴权，与实测不符）。最值得新增的 #10 nmobi 与 #2 lx-kw 实测全通，可信度高；#14 yunzhiapi 若集成需先补 token 依赖说明。
...
| `api.nki.pw/API/music_open_api.php` | `_parsewithnkiapi` | l2 VIP | `apikey`（2 个）；一次返回 `song_play_url_sq/pq/accom/hq/standard/fq` 全音质 | ⚠️ 本次 25s 超时未确认（此前实测 sq 段为 m4a） |
